...
let currentRawHTML = "";
let injectionMap = {};
let detectedInjectionTargets = [];
let currentTickerText = ""; // legacy, unused
let overlayHasTickerPlaceholder = false; // legacy, unused
...
function renderHTMLLayout(htmlString) {
    ...
    detectInjectionTargets();
    applyInjections();

    overlayActive = true;
}

function detectInjectionTargets() {
    const container = $('hiddenOverlayContainer');
    if (!container) return;
    const els = Array.from(container.querySelectorAll('[id]'));
    detectedInjectionTargets = els
        .filter(el => {
            const tag = el.tagName.toLowerCase();
            if (!['div','span','p','marquee'].includes(tag)) return false;
            const txt = (el.textContent || '').trim();
            return txt.length <= 300;
        })
        .map(el => el.id);

    const select = $('injectTargetSelect');
    if (!select) return;
    select.innerHTML = '<option value="">Detected zones will appear here...</option>';
    detectedInjectionTargets.forEach(id => {
...
function applyInjections() {
    const container = $('hiddenOverlayContainer');
    if (!container) return;
    Object.keys(injectionMap).forEach(id => {
        const el = container.querySelector('#' + CSS.escape(id));
        if (el) {
            el.innerHTML = injectionMap[id];
        }
    });
}
...
(function(){
    ...
    const injectTargetSelect = $('injectTargetSelect');
    const injectTargetId = $('injectTargetId');
    const injectValue = $('injectValue');
    const btnInjectContent = $('btnInjectContent');

    if (injectTargetSelect && injectTargetId) {
        injectTargetSelect.onchange = () => {
            if (!injectTargetSelect.value) return;
            injectTargetId.value = injectTargetSelect.value;
        };
    }

    if (btnInjectContent && injectTargetId && injectValue) {
        btnInjectContent.onclick = () => {
            const targetId = injectTargetId.value.trim();
            const content = injectValue.value;
            if (!targetId) {
                alert("Please specify a target ID (from your overlay HTML).");
                return;
            }
            injectionMap[targetId] = content;
            applyInjections();
        };
    }
