...
function loadOverlayFromHTML(htmlString) {
    // Called when a *new* overlay file or preset is loaded
    overlayFields = {};
    overlayFieldOrder = [];
    currentRawHTML = "";
...
function detectOverlayFieldsFromHTML(htmlString) {
    const fields = [];
...
function rebuildOverlayEditorUI() {
    const panel = $('overlayEditorContainer');
    if (!panel) return;

    const names = overlayFieldOrder.filter(n => !(n.toLowerCase() === 'viewers' || n.toLowerCase() === 'guests' || n.toLowerCase() === 'title' || n.toLowerCase() === 'chat'));
    if (!names.length) {
    ...
    let html = '';
    html += '<label style="color:var(--accent);">📄 Live Overlay Editor</label>';
    html += '<p style="font-size:0.7rem; color:var(--muted); margin-bottom:5px;">Detected fields in your overlay HTML.</p>';
    html += '<div id="overlayEditorFields"></div>';
    html += '<button id="overlayEditorUpdateBtn" class="btn small primary full-width" style="margin-top:8px;">Update Overlay</button>';

    panel.innerHTML = html;

    const fieldsContainer = document.getElementById('overlayEditorFields');
    ...
    names.forEach(name => {
        ...
        const isTicker = lower.includes('ticker');
        ...
        input.setAttribute('data-overlay-field', name);
        input.value = overlayFields[name] || '';
    ...
    const updateBtn = $('overlayEditorUpdateBtn');
    if (updateBtn) {
        updateBtn.onclick = () => {
            // read back all values
            const inputs = fieldsContainer.querySelectorAll('[data-overlay-field]');
            inputs.forEach(inp => {
                const name = inp.getAttribute('data-overlay-field');
                overlayFields[name] = inp.value;
            });
...
function renderHTMLLayout(htmlString) {
    ...
    // currentRawHTML holds the original template for this overlay
    if (!currentRawHTML) {
    ...
    // Detect overlay fields from the raw HTML
    const detectedNames = detectOverlayFieldsFromHTML(currentRawHTML);
    detectedNames.forEach(name => {
        if (!overlayFieldOrder.includes(name)) overlayFieldOrder.push(name);
        if (!(name in overlayFields)) overlayFields[name] = '';
    });

    let processedHTML = currentRawHTML
        .replace(/{{viewers}}/gi, viewerCount)
        .replace(/{{guests}}/gi, guestCount)
        .replace(/{{title}}/gi, streamTitle)
        .replace(/{{chat}}/gi, chatHTML);

    // Apply custom overlay fields
    overlayFieldOrder.forEach(name => {
        const lower = name.toLowerCase();
        let value = overlayFields[name] || '';

        if (!value) {
            if (lower === 'streamtitle') value = streamTitle;
        }

        if (lower.includes('ticker')) {
            currentTickerText = value;
        }

        const re = new RegExp('{{\\s*' + name + '\\s*}}', 'g');
        processedHTML = processedHTML.replace(re, value || '');
    });

    // Backwards-compat: generic {{TICKER}} placeholder
    if (currentTickerText) {
        processedHTML = processedHTML.replace(/{{\s*TICKER\s*}}/gi, currentTickerText);
    }

    container.innerHTML = processedHTML;
    rebuildOverlayEditorUI();
    overlayActive = true;
}
...
(function(){
    ...
    if (htmlOverlayInput) {
        htmlOverlayInput.onchange = (e) => {
            ...
            const r = new FileReader();
            r.onload = (ev) => {
                loadOverlayFromHTML(ev.target.result);
                const overlayStatus = $('overlayStatus');
                if (overlayStatus) overlayStatus.textContent = "[Loaded overlay from file]";
            };
            r.readAsText(f);
        };
    }

    // Preset overlay loader (from /tools/overlays/*.html)
    ...
    if (presetSelect && btnLoadPreset) {
        btnLoadPreset.onclick = async () => {
            ...
            try {
                ...
                loadOverlayFromHTML(html);
                const overlayStatus = $('overlayStatus');
                if (overlayStatus) overlayStatus.textContent = "[Loaded preset: " + file + "]";
            } catch (err) {
