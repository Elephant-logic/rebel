...
let overlayHasTickerPlaceholder = false; // legacy ticker support

// Live Injection map: ID -> content
let injectionMap = {};
...
function renderHTMLLayout(htmlString) {
    ...
    // Load base HTML into hidden container
    container.innerHTML = processedHTML;

    // Apply any saved Live Injections after base load
    applyInjections();

    overlayActive = true;
}

// Apply all saved Live Injections into the hidden HTML overlay container
...
// === Overlay & HTML Injection Controls ===
(function () {
    // Upload HTML layout for overlay
    const htmlOverlayInput = $('htmlOverlayInput');
    if (htmlOverlayInput) {
        htmlOverlayInput.onchange = (e) => {
            const f = e.target.files[0];
    ...
    if (overlayToggleBtn) {
        overlayToggleBtn.onclick = () => {
            ...
            if (!overlayActive) {
                ...
                // Re-render base HTML + reapply injections
                renderHTMLLayout(currentRawHTML);
            }
        };
    }

    // ⚡ Live Injection wiring
    ...
    if (btnInjectContent && injectTargetId && injectValue) {
        window.injectIntoOverlay = () => {
            const targetId = injectTargetId.value.trim();
            const content  = injectValue.value;

            if (!targetId) {
                alert("Please specify a Target ID from your HTML.");
                return;
            }

            // Save and apply injection
            injectionMap[targetId] = content;
            applyInjections();
        };

        btnInjectContent.onclick = window.injectIntoOverlay;
    }
})();
