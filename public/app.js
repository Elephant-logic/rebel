...
function ensureMixerStream() {
    ensureCanvasStream(); //
    if (!mixerStream) {
        mixerStream = new MediaStream(); //
    }

    const videoTrack = canvasStream.getVideoTracks()[0]; //
    if (videoTrack && !mixerStream.getVideoTracks().includes(videoTrack)) {
        mixerStream.addTrack(videoTrack); //
    }

    const audioTrack = localStream?.getAudioTracks()[0]; //
    if (audioTrack && !mixerStream.getAudioTracks().includes(audioTrack)) {
...
canvasStream = ensureCanvasStream(); //
drawMixer(); //
