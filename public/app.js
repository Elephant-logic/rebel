...
// PATCH: Graphics Overlay & QR State
...
function drawMixer() {
    ...
    if (mixerLayout === 'SOLO') {
        ...
        // Picture-in-Picture (Host Full + Guest Small Overlay)
    ...
    // PATCH: DRAW LOWER THIRD OVERLAY
    if (showOverlay) {
        ctx.fillStyle = 'rgba(74, 243, 163, 0.9)';
        ctx.fillRect(50, 950, 700, 80);
        ctx.fillStyle = '#000';
        ctx.font = 'bold 44px sans-serif';
        ctx.textAlign = "left";
        ctx.fillText(overlayText || "REBEL STREAM LIVE", 80, 1005);
    }

    // PATCH: VIEWER COUNT OVERLAY
    ctx.fillStyle = 'rgba(0,0,0,0.6)';
    ctx.roundRect(1650, 40, 230, 60, 15); ctx.fill();
    ctx.fillStyle = '#ff4b6a';
    ctx.beginPath(); ctx.arc(1685, 70, 10, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = '#fff'; ctx.font = 'bold 30px sans-serif';
    ctx.fillText(`${viewerCount} VIEWERS`, 1710, 80);
...
// PATCH: Toggle Graphic Overlay
window.setOverlay = (visible) => {
    showOverlay = visible;
    overlayText = $('overlayText')?.value || "";
};

// PATCH: QR Code Generator
function generateQR(url) {
    const container = $('qrContainer');
    if (!container) return;
    container.style.display = 'block';
    if (!qr) {
        qr = new QRCode($("qrcode"), { text: url, width: 128, height: 128 });
    } else {
        qr.makeCode(url);
    }
}
...
const contents = { 
    ...
    files: $('contentFiles'), 
    users: $('contentUsers'),
    graphics: $('contentGraphics') // PATCH
};
...
Object.keys(tabs).forEach(key => {
    if(tabs[key]) tabs[key].onclick = () => switchTab(key);
});
...
$('sendFileBtn').onclick = () => {
    ...
    if(file.size > 1024 * 1024) return alert("1MB Limit");
...
if ($('arcadeInput')) {
    $('arcadeInput').onchange = () => {
        ...
        if(!resendBtn) {
            resendBtn = document.createElement('button'); resendBtn.id = 'resendToolBtn'; resendBtn.textContent = 'Force Resend Tool';
            resendBtn.className = 'btn small secondary full-width'; resendBtn.style.marginTop = '5px';
            resendBtn.onclick = () => Object.values(viewerPeers).forEach(pc => pushFileToPeer(pc, activeToolboxFile));
...
function renderUserList() {
    ...
    latestUserList.forEach(u => {
        ...
        actionBtn.textContent = isCalling ? 'End Call' : 'Call';
...
function addRemoteVideo(id, stream) {
    ...
    if (!d) {
        ...
        d.appendChild(v); d.appendChild(h2); $('videoGrid').appendChild(d);
    }
