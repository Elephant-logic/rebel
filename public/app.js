...
let myId = null;
let iAmHost = false;
let wasHost = false; // PATCH: Track previous role for migration
let latestUserList = [];
...
// PATCH: Move actual logic to a function for migration auto-start
...
socket.on('user-joined', ({ id, name }) => {
    if (iAmHost && isPrivateMode) {
        ...
        if (!isAllowed) { socket.emit('kick-user', id); return; }
    }
...
socket.on('role', ({ isHost }) => {
    wasHost = iAmHost; // Store what we were
    iAmHost = isHost;
    ...
    if (isHost && !wasHost && currentRoom) {
        appendChat($('chatLogPrivate'), 'System', '⚠️ HOST MIGRATION: You are now broadcasting.', Date.now());
        handleStartStream();
    }
    renderUserList();
});
...
if ($('updateTitleBtn')) $('updateTitleBtn').onclick = () => { if($('streamTitleInput').value.trim()) socket.emit('update-stream-title', $('streamTitleInput').value.trim()); };
if ($('updateSlugBtn')) $('updateSlugBtn').onclick = () => { if($('slugInput').value.trim()) updateLink($('slugInput').value.trim()); };
...
if ($('addGuestBtn')) $('addGuestBtn').onclick = () => {
    ...
    if (name && !allowedGuests.includes(name)) { allowedGuests.push(name); renderGuestList(); $('guestNameInput').value = ''; }
};
...
function renderGuestList() {
    ...
    allowedGuests.forEach(name => {
        const tag = document.createElement('span'); tag.style.cssText = "background:var(--accent); color:#000; padding:2px 6px; border-radius:4px; font-size:0.7rem;";
        tag.textContent = name; display.appendChild(tag);
    });
}
...
function sendPublic() { if($('inputPublic').value.trim() && currentRoom) { socket.emit('public-chat', { room: currentRoom, name: userName, text: $('inputPublic').value.trim() }); $('inputPublic').value = ''; } }
$('btnSendPublic').onclick = sendPublic;
$('inputPublic').onkeydown = e => { if(e.key === 'Enter') sendPublic(); };

function sendPrivate() { if($('inputPrivate').value.trim() && currentRoom) { socket.emit('private-chat', { room: currentRoom, name: userName, text: $('inputPrivate').value.trim() }); $('inputPrivate').value = ''; } }
$('btnSendPrivate').onclick = sendPrivate;
$('inputPrivate').onkeydown = e => { if(e.key === 'Enter') sendPrivate(); };
...
$('sendFileBtn').onclick = () => {
    ...
    if(file.size > 1024 * 1024) return alert("File too large for chat. Use 'Arcade'.");
...
socket.on('file-share', d => {
    ...
    const info = document.createElement('div'); const b = document.createElement('strong'); b.textContent = d.name; info.appendChild(b); info.appendChild(document.createTextNode(` shared: ${d.fileName}`));
    ...
    div.appendChild(info); div.appendChild(link);
...
if ($('arcadeInput')) $('arcadeInput').onchange = () => {
    ...
    if(!resendBtn) {
        resendBtn = document.createElement('button'); resendBtn.id = 'resendToolBtn'; resendBtn.textContent = 'Force Resend Tool'; resendBtn.className = 'btn small secondary full-width'; resendBtn.style.marginTop = '5px';
        resendBtn.onclick = () => { Object.values(viewerPeers).forEach(pc => pushFileToPeer(pc, activeToolboxFile)); alert("Tool resent."); };
...
function renderUserList() {
    ...
    latestUserList.forEach(u => {
        ...
        if (callPeers[u.id]) { actionBtn.textContent = 'End Call'; actionBtn.style.cssText = 'border-color:var(--danger); color:var(--danger)'; actionBtn.onclick = () => endPeerCall(u.id); }
        else { actionBtn.textContent = 'Call'; actionBtn.onclick = () => socket.emit('ring-user', u.id); }
        actionsDiv.appendChild(actionBtn);
        if (callPeers[u.id] && iAmHost) {
            const selBtn = document.createElement('button'); selBtn.className = 'action-btn'; selBtn.textContent = (activeGuestId === u.id) ? 'Selected' : 'Select';
            selBtn.onclick = () => { activeGuestId = u.id; renderUserList(); setActiveGuest(u.id); }; actionsDiv.appendChild(selBtn);
        }
        if (iAmHost) {
            const kickBtn = document.createElement('button'); kickBtn.className = 'action-btn kick'; kickBtn.textContent = 'Kick';
            kickBtn.onclick = () => socket.emit('kick-user', u.id); actionsDiv.appendChild(kickBtn);
        }
...
if ($('openStreamBtn')) $('openStreamBtn').onclick = () => { if($('streamLinkInput').value) window.open($('streamLinkInput').value, '_blank'); };
