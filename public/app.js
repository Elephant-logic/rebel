...
// ======================================================
// 3. CANVAS MIXER ENGINE (PATCHED: 30FPS + Audio Glow Loop)
// ======================================================
...
function drawMixer(timestamp) {
    ...
    // --- UPGRADE: Audio Speaking Glow Loop ---
    Object.keys(audioAnalysers).forEach(id => {
        const entry = audioAnalysers[id];
        ...
        const container = (id === 'local') ? $('localContainer') : document.getElementById(`vid-${id}`);
        if (container) {
            if (avg > 25) { // Threshold for speaking
...
async function applyBitrateConstraints(pc) {
    ...
    if (videoSender) {
        try {
            ...
            console.log("🚀 Bitrate locked to 2500kbps");
...
// ======================================================
// 9. P2P CALLING & UPGRADED MIX BRIDGE
// ======================================================
...
// ======================================================
// 16. USER LIST & MIXER SELECTION (UPDATED: Stats & Bridge)
// ======================================================
        ...
        users.forEach(u => {
            ...
            if (isCalling) {
                ...
                callBtn.onclick = () => {
                    ...
                    activeGuestId = u.id; // Push to mixer
                    mixerLayout = 'PIP';  
                    renderUserList();
                }; 
            }
