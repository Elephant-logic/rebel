...
let lastSharedPayload = null; // Store for resending
...
resendBtn.className = 'btn secondary full-width';
resendBtn.style.marginTop = '8px';
resendBtn.style.display = 'none'; // Hidden by default
resendBtn.textContent = '🔄 Resend Last File';
uploadArea.appendChild(resendBtn);

resendBtn.onclick = () => {
    if (lastSharedPayload && currentRoom) {
        if(confirm(`Resend "${lastSharedPayload.fileName}" to everyone?`)) {
            socket.emit('file-share', lastSharedPayload);
            // Quick flash to confirm
            const oldT = resendBtn.textContent;
            resendBtn.textContent = 'Sent!';
            setTimeout(() => resendBtn.textContent = oldT, 2000);
        }
    }
};
...
$('sendFileBtn').addEventListener('click', () => {
    ...
    if(file.size > MAX_SIZE) {
        alert(`File too large for chat. Limit is 20MB. (Your file: ${(file.size/1024/1024).toFixed(2)}MB)`);
        return;
    }
    ...
    reader.onload = () => {
        // Store payload for potential resend
        lastSharedPayload = { 
            room: currentRoom, 
            name: userName, 
            fileName: file.name, 
        ...
        resendBtn.textContent = `🔄 Resend "${file.name}"`;
    };
...
if (arcadeInput) {
    arcadeInput.addEventListener('change', () => {
        ...
        let resendToolBtn = document.getElementById('resendToolBtn');
        if(!resendToolBtn) {
            resendToolBtn = document.createElement('button');
            resendToolBtn.id = 'resendToolBtn';
            resendToolBtn.textContent = 'Force Resend Tool';
            resendToolBtn.className = 'btn small secondary full-width';
            resendToolBtn.style.marginTop = '5px';
            resendToolBtn.onclick = () => {
            ...
            $('arcadeStatus').parentNode.appendChild(resendToolBtn);
        }
