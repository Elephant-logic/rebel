...
// --- HTML LAYOUT ENGINE WITH STATS INJECTION ---
function renderHTMLLayout(htmlString) {
    ...
    // Replace placeholders: {{viewers}}, {{guests}}, {{title}}
    ...
    const svg = `
        <svg xmlns="http://www.w3.org/2000/svg" width="1920" height="1080">
            <foreignObject width="100%" height="100%">
                <div xmlns="http://www.w3.org/1999/xhtml" class="layout-${mixerLayout}" style="width:100%; height:100%; margin:0; padding:0; color:white;">
                    ${processedHTML}
                </div>
            </foreignObject>
        </svg>`;
...
socket.on('call-request-received', ({ id, name }) => {
    // Add a system message to Private Chat to alert the Host
    ...
    // Refresh user list to show the ✋ icon
    renderUserList();
});
...
if (joinBtn) {
    joinBtn.onclick = () => {
        ...
        socket.connect();
        // Index.html users are always "Guests", not "Viewers"
    ...
    const renderGroup = (label, users) => {
        ...
        users.forEach(u => {
            ...
            if (isCalling) {
                ...
                // If viewer is requesting call, make button prominent
