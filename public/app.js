...
function renderHTMLLayout(htmlString) {
    ...
    // fallbacks from overlayConfig
    const ov = overlayConfig || {};
    const ovStreamTitle = ov.streamTitle || streamTitle;
    const ovDjName = ov.djName || "";
    const ovNowPlaying = ov.nowPlaying || "";
    const ovTickerText = ov.tickerText || currentTickerText || "";
    const ovAccentColor = ov.accentColor || "#ff4b6a";
    const ovTickerSpeed = ov.tickerSpeedSec != null ? ov.tickerSpeedSec : 25;
    const ovScene = ov.sceneMode || "FULL";

    let processedHTML = htmlString
        .replace(/{{viewers}}/g, viewerCount)
        .replace(/{{guests}}/g, guestCount)
        .replace(/{{title}}/g, ovStreamTitle)
        .replace(/{{chat}}/g, chatHTML)
        .replace(/{{\s*STREAMTITLE\s*}}/gi, ovStreamTitle)
        .replace(/{{\s*DJNAME\s*}}/gi, ovDjName)
        .replace(/{{\s*NOWNPLAYING\s*}}/gi, ovNowPlaying)
        .replace(/{{\s*NOWPLAYING\s*}}/gi, ovNowPlaying)
        .replace(/{{\s*TICKERTEXT\s*}}/gi, ovTickerText)
        .replace(/{{\s*ACCENTCOLOR\s*}}/gi, ovAccentColor)
        .replace(/{{\s*TICKERSPEED\s*}}/gi, String(ovTickerSpeed))
        .replace(/{{\s*SCENE\s*}}/gi, ovScene);
    ...
    if (overlayHasTickerPlaceholder && ovTickerText) {
        processedHTML = processedHTML.replace(/{{\s*TICKER\s*}}/gi, ovTickerText);
    }

    container.innerHTML = processedHTML;

    try {
        container.style.setProperty('--accent-color', ovAccentColor);
        container.setAttribute('data-scene', ovScene);
    } catch (e) {
...
(function(){
    ...
    // Preset overlays from /tools/overlays/*
    const overlayPresetsBase = "/tools/overlays/";
    const presetSelect = $('overlayPresetSelect');
    const btnLoadPreset = $('btnLoadOverlayPreset');

    if (presetSelect && btnLoadPreset) {
        btnLoadPreset.onclick = async () => {
            const file = presetSelect.value;
            if (!file) return;
            try {
                const res = await fetch(overlayPresetsBase + file);
                if (!res.ok) throw new Error("HTTP " + res.status);
                const html = await res.text();
                renderHTMLLayout(html);
                const overlayStatus = $('overlayStatus');
                if (overlayStatus) {
                    overlayStatus.textContent = `[Loaded preset: ${file}]`;
                }
            } catch (err) {
                console.error("Preset load failed", err);
                alert("Could not load preset overlay: " + err.message);
            }
        };
    }

    // Legacy ticker input hook (kept for compatibility)
    ...
    if (tickerInput && tickerUpdateBtn) {
        tickerUpdateBtn.onclick = () => {
            ...
            overlayConfig.tickerText = currentTickerText;
    ...
    // Live Overlay Editor controls (Full / Ticker + fields)
    const sceneFullBtn = $('overlaySceneFullBtn');
    const sceneTickerBtn = $('overlaySceneTickerBtn');
    const overlayStreamTitleInput = $('overlayStreamTitleInput');
    const overlayDjNameInput = $('overlayDjNameInput');
    const overlayNowPlayingInput = $('overlayNowPlayingInput');
    const overlayTickerTextInput = $('overlayTickerTextInput');
    const overlayAccentColorInput = $('overlayAccentColorInput');
    const overlayTickerSpeedInput = $('overlayTickerSpeedInput');
    const overlayTickerSpeedLabel = $('overlayTickerSpeedLabel');
    const overlayUpdateBtn = $('overlayUpdateBtn');

    function refreshSceneButtons() {
        if (sceneFullBtn && sceneTickerBtn) {
            if (overlayConfig.sceneMode === 'FULL') {
                sceneFullBtn.classList.add('primary');
                sceneFullBtn.classList.remove('secondary');
                sceneTickerBtn.classList.remove('primary');
                sceneTickerBtn.classList.add('secondary');
            } else {
                sceneTickerBtn.classList.add('primary');
                sceneTickerBtn.classList.remove('secondary');
                sceneFullBtn.classList.remove('primary');
                sceneFullBtn.classList.add('secondary');
            }
        }
    }

    if (sceneFullBtn) {
        sceneFullBtn.onclick = () => {
            overlayConfig.sceneMode = 'FULL';
            refreshSceneButtons();
            if (overlayActive && currentRawHTML) renderHTMLLayout(currentRawHTML);
        };
    }
    if (sceneTickerBtn) {
        sceneTickerBtn.onclick = () => {
            overlayConfig.sceneMode = 'TICKER';
            refreshSceneButtons();
            if (overlayActive && currentRawHTML) renderHTMLLayout(currentRawHTML);
        };
    }

    if (overlayTickerSpeedInput && overlayTickerSpeedLabel) {
        overlayTickerSpeedInput.oninput = () => {
            const v = Number(overlayTickerSpeedInput.value || 25);
            overlayConfig.tickerSpeedSec = v;
            overlayTickerSpeedLabel.textContent = `Ticker Speed: ${v}s`;
            if (overlayActive && currentRawHTML) renderHTMLLayout(currentRawHTML);
        };
    }

    if (overlayUpdateBtn) {
        overlayUpdateBtn.onclick = () => {
            overlayConfig.streamTitle = overlayStreamTitleInput && overlayStreamTitleInput.value
                ? overlayStreamTitleInput.value
                : overlayConfig.streamTitle;
            overlayConfig.djName = overlayDjNameInput ? overlayDjNameInput.value : overlayConfig.djName;
            overlayConfig.nowPlaying = overlayNowPlayingInput ? overlayNowPlayingInput.value : overlayConfig.nowPlaying;
            overlayConfig.tickerText = overlayTickerTextInput ? overlayTickerTextInput.value : overlayConfig.tickerText;
            overlayConfig.accentColor = overlayAccentColorInput && overlayAccentColorInput.value
                ? overlayAccentColorInput.value
                : overlayConfig.accentColor;

            currentTickerText = overlayConfig.tickerText || "";
    ...
    // Initialize defaults in editor fields if empty
    if (overlayStreamTitleInput && !overlayStreamTitleInput.value) {
        overlayStreamTitleInput.value = $('streamTitleInput') ? $('streamTitleInput').value : "Rebel Stream: Live";
    }
    if (overlayDjNameInput && !overlayDjNameInput.value) {
        overlayDjNameInput.value = overlayConfig.djName || "";
    }
    if (overlayNowPlayingInput && !overlayNowPlayingInput.value) {
        overlayNowPlayingInput.value = overlayConfig.nowPlaying || "";
    }
    if (overlayTickerTextInput && !overlayTickerTextInput.value) {
        overlayTickerTextInput.value = overlayConfig.tickerText || currentTickerText || "Welcome to the Rebel Stream";
    }
    if (overlayAccentColorInput && !overlayAccentColorInput.value) {
        overlayAccentColorInput.value = overlayConfig.accentColor || "#ff4b6a";
    }
    if (overlayTickerSpeedInput && !overlayTickerSpeedInput.value) {
        overlayTickerSpeedInput.value = String(overlayConfig.tickerSpeedSec || 25);
        if (overlayTickerSpeedLabel) {
            overlayTickerSpeedLabel.textContent = `Ticker Speed: ${overlayTickerSpeedInput.value}s`;
        }
    }
    refreshSceneButtons();
