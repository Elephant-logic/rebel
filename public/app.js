    ...
    channel.onopen = async () => {
        ...
        const sendLoop = () => {
            ...
            if (offset < buffer.byteLength) {
                ...
                console.log(`[Arcade] Complete: ${file.name}`);
...
// Hangup Button (Disconnects P2P only, keeps Camera ON)
...
// Room Update (includes Title from Server)
socket.on('room-update', ({ locked, streamTitle, ownerId, users }) => {
    latestUserList = users;
    currentOwnerId = ownerId;
    
    // UPDATE UI WITH TITLE FROM SERVER
...
// ==========================================
// 10. HOST CONTROLS (TITLE, SLUG, VIP)
// ==========================================
...
// Allow pressing "Enter" on the title box
if ($('streamTitleInput')) {
...
if (togglePrivateBtn) {
    togglePrivateBtn.addEventListener('click', () => {
        ...
        if (isPrivateMode) {
            // Kick currently connected strangers
