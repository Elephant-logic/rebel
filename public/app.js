...
// WebRTC peer maps
const viewerPeers = {};  // for broadcast viewers
const callPeers = {};    // for 1:1 calls
    ...
    if (mixerLayout === 'SOLO') {
        ...
        Object.keys(callPeers).forEach(id => {
            ...
            if (el) {
                ...
                if (v && v.readyState === 4) participants.push(v);
            }
        });
...
// Initialize canvas stream & mixer loop
...
function renderHTMLLayout(htmlString) {
    ...
    overlayActive = true;
    if ($('overlayStatus')) $('overlayStatus').textContent = "Overlay Active";
}
...
async function startLocalMedia() {
    ...
    try {
        ...
        const constraints = {
            audio: { deviceId: audioSource && audioSource.value ? { exact: audioSource.value } : undefined },
            video: {
        ...
        // Update outgoing tracks to viewers/call peers
        ...
        const updateViewerPC = (pc) => {
            ...
            if (vSender && mixedVideoTrack) vSender.replaceTrack(mixedVideoTrack);
        ...
        Object.values(callPeers).forEach(p => {
            ...
            const vSender = senders.find(s => s.track && s.track.kind === 'video');
            if (vSender && mainStream.getVideoTracks()[0]) vSender.replaceTrack(mainStream.getVideoTracks()[0]);
            const aSender = senders.find(s => s.track && s.track.kind === 'audio');
            if (aSender && finalAudioTrack) aSender.replaceTrack(finalAudioTrack);
        });
            ...
            try {
                ...
                Object.values(callPeers).forEach(p => {
                    p.pc.getSenders().forEach(s => {
                        if (s.track && s.track.kind === 'video' && st) s.replaceTrack(st);
...
async function handleStartStream() {
    ...
    // Connect existing users as viewers
...
socket.on('ring-alert', async ({ from, fromId }) => {
    if (confirm(`Incoming call from ${from}. Accept?`)) {
        await callPeer(fromId, true);
    }
});

async function callPeer(targetId, isAnswerSide = false) {
    if (!localStream) await startLocalMedia();
...
async function connectViewer(targetId) {
    ...
    // optional control channel if needed later
    ...
    // broadcast: canvas video + host audio
    ...
    // send arcade file if there is an active toolbox item
...
if ($('joinBtn')) {
    $('joinBtn').onclick = () => {
        ...
        updateLink(); // use actual room (see patched function below)
        startLocalMedia();
    };
}
...
// PATCHED: always use the actual room you're in, not the slug alone
function updateLink() {
    const url = new URL(window.location.href);
    const roomVal = ($('roomInput') ? $('roomInput').value.trim() : '') || currentRoom || '';
    url.pathname = url.pathname.replace('index.html', '') + 'view.html';
    url.search = `?room=${encodeURIComponent(roomVal)}`;
...
socket.on('role', ({ isHost, streamTitle }) => {
    ...
    if ($('localContainer')) {
        const h = $('localContainer').querySelector('h2');
        if (h) h.textContent = isHost ? 'You (Host)' : 'You';
    }
    if ($('hostControls')) {
...
if ($('updateSlugBtn')) {
    $('updateSlugBtn').onclick = () => {
        // With the patched updateLink, slug is just cosmetic for now.
        // We could store & show it somewhere; for now we simply regenerate link.
        updateLink();
    };
}
...
if ($('togglePrivateBtn')) {
    $('togglePrivateBtn').onclick = () => {
        ...
        // Kick non-allowed users when turning private on
        if (isPrivateMode) {
            latestUserList.forEach(u => {
...
if ($('sendFileBtn')) {
    $('sendFileBtn').onclick = () => {
        ...
        if (f.size > 10 * 1024 * 1024) {
            alert("File too large (Limit: 10MB). Use 'Arcade' for bigger P2P transfers.");
            return;
        }
...
if (htmlOverlayInput) {
    htmlOverlayInput.onchange = (e) => {
        ...
        if (!f) return;
        const r = new FileReader();
        r.onload = (ev) => renderHTMLLayout(ev.target.result);
        r.readAsText(f);
    };
}
...
window.clearOverlay = () => {
    ...
    if ($('overlayStatus')) $('overlayStatus').textContent = "[No Overlay]";
};
...
// Expose some helpers to window for buttons
