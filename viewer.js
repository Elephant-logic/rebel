...
function appendChat(name, text, ts = Date.now()) {
    ...
    const who = name === 'You' ? `${name}` : `${name}`;
    line.innerHTML = `${who} ${time}: ${text}`;
...
async function createViewerPC() {
    ...
    pc.onconnectionstatechange = () => {
        if (!pc) return;
        if ((pc.connectionState === 'failed' || pc.connectionState === 'disconnected') && (!viewerVideo || !viewerVideo.srcObject)) {
